...
router.post("/img/upload", upload.single("image"), async (req, res) => {
  try {
    ...
    res.json({
      processedImageUrl: `https://server-process.vercel.app/images/api/images/latest`,
      imageDetails: {
...
router.get("/api/download/:imageId", async (req, res) => {
  try {
...
router.get("/api/images/latest", async (req, res) => {
  try {
